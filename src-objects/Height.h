...
public:
    ...
    ~Height();
    bool    operator ==(Height &h)  const { return isEqual(h);                      }
    bool    operator !=(Height &h)  const { return !isEqual(h);                     }
    bool    operator <(Height &h)   const { return (!isEqual(h) && !isGreater(h));  }
    bool    operator <=(Height &h)  const { return !isGreater(h);                   }
    bool    operator >(Height &h)   const { return isGreater(h);                    }
    bool    operator >=(Height &h)  const { return (isGreater(h) || isEqual(h));    }
